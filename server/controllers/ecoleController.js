...
const addStudent = async (req, res) => {
  const { nom, prenom, email, telephone, filiere, annee_scolarite, id_respo } =
    req.body;
  try {
    ...
    const query = `
      INSERT INTO Etudiant (nom, prenom, email, telephone, mdp, filiere, annee_scolarite, id_ecole, id_respo, statut_etudiant, recuperation)
    ...
    res
      .status(201)
      .json({ message: "Student added successfully", generatedPassword });
  } catch (error) {
  ...
  try {
    ...
    if (selected) {
      ...
      if (deleteResult.rowsAffected === 0) {
        return res
          .status(400)
          .json({ error: "Entreprise was not selected previously" });
      }
